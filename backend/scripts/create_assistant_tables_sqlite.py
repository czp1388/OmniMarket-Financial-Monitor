"""
使用SQLite创建助手模式数据库表（开发环境）
运行: python backend/scripts/create_assistant_tables_sqlite.py
"""
import sys
import os
...
def create_tables():
    """使用SQLite创建所有助手模式相关表"""
    try:
        logger.info("开始创建助手模式数据库表（SQLite）...")
        ...
        logger.info("数据库文件: omnimarket.db")
        logger.info("创建的表：")
        logger.info("  - users (用户)")
